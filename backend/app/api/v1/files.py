...
@router.post("/upload")
async def upload_file(file: UploadFile = File(...), subdir: str = "", user=Depends(get_current_user)):
    if not user:
...
@router.get("/list")
async def list_files(path: Optional[str] = Query(None), user=Depends(get_current_user)):
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    rel = path or ""
    try:
...
async def read_file(file_id: str, user=Depends(get_current_user)):
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
