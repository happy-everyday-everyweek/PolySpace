...
async def update_board(board_id: int, update: BoardUpdate):
...
async def get_archived_cards(board_id: int):
...
async def update_column(column_id: int, update: ColumnUpdate):
    updates = {k: v for k, v in update.model_dump().items() if v is not None}
...
async def reorder_columns(board_id: int, reorder: ColumnReorder):
...
async def move_card(card_id: int, move: CardMove):
...
async def unarchive_card(card_id: int):
...
async def card_to_todo(req: CardToTodoRequest):
...
async def get_card_linked_todos(card_id: int):
