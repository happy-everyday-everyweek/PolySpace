# ruff: noqa: E501
IMPROVE_ARCHITECTURE_METHODOLOGY = """# Improve Codebase Architecture
...
**Use CONTEXT.md vocabulary for the domain, and the glossary vocabulary for the architecture.** If `CONTEXT.md` defines "Order," talk about "the Order intake module" — not "the FooBarHandler," and not "the Order service."
...
2. **Local-substitutable** — Dependencies with local test stand-ins (PGLite for Postgres, in-memory filesystem). Deepenable if the stand-in exists.
3. **Remote but owned (Ports & Adapters)** — Your own services across a network boundary. Define a port at the seam. Tests use an in-memory adapter.
4. **True external (Mock)** — Third-party services you don't control. The deepened module takes the external dependency as an injected port; tests provide a mock adapter.
...
def execute(target_area: str = "", goals: str = "") -> dict:
    return {
        "skill": "improve-codebase-architecture",
        "methodology": IMPROVE_ARCHITECTURE_METHODOLOGY,
        "target_area": target_area,
        "goals": goals,
        "instructions": (
            "Apply the improve-codebase-architecture methodology. Explore the codebase "
            "for friction, present deepening opportunities, and guide the user through "
            "a grilling loop for the chosen candidate. Use domain glossary vocabulary."
        ),
    }
